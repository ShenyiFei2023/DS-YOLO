...
class C3_PP(nn.Module):
    """CSP Bottleneck with 3 convolutions."""
    def __init__(self, c1, c2, n=1, tcr=0.25,shortcut=True, g=1, e=0.5):
        ...
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(2 * c_, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.Sequential(*(PartiallyTransformerBlock(c_, tcr,shortcut) for _ in range(n)))
...
class C3k_PP(C3_PP):
    ...
    def __init__(self, c1, c2, n=1,tcr=0.25, shortcut=True, g=1, e=0.5, k=3):
        """Initializes the C3k module with specified channels, number of layers, and configurations."""
        super().__init__(c1, c2, n, tcr,shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(PartiallyTransformerBlock(c_, tcr, shortcut) for _ in range(n)))


class C3k2_PP(CSP_PPA):
    def __init__(self, c1, c2, n=1 ,tcr=0.25,c3k=True, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, tcr,shortcut, g, e)
        self.m = nn.ModuleList(
            C3k_PP(self.c, self.c, 2,tcr, shortcut=shortcut) if c3k else Bottleneck(self.c, self.c, shortcut, g) for _ in range(n)
        )
